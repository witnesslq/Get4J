...
CREATE TABLE `page` (
  ...
  `SEED_NAME` VARCHAR(20) COMMENT '种子名称',
  `SITE_URL` VARCHAR(2048) NOT NULL COMMENT '平台站点地址',
  `FETCH_URL` VARCHAR(2048) NOT NULL COMMENT '每次抓取的url地址',
  `TITLE` VARCHAR(1000) COMMENT '网页标题',
  `FETCH_TIME` VARCHAR(20)  COMMENT '抓取时间',
  `FETCH_CONTENT` LONGTEXT  COMMENT 'Html或Json内容',
  `RESOURCES_URL` LONGTEXT  COMMENT 'Html或Json文件中的URL',
